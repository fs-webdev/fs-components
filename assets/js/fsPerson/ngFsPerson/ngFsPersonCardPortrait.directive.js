/**
 * Render a person's portrait with their gender and vitals (name, lifeSpan, and id) in the format for the person card.
 *
 * @since 1.1.0
 */
angular.module('ngFsModules')
       .directive('fsPersonCardPortrait', function() {
  return {
    restrict: 'E',
    replace: true,
    template: templateList.fsPersonCardPortrait,
    scope: {
      person: '=',
      config: '&'
    },
    link: function(scope, element, attrs) {
      ...
      scope = fsPersonCardPortraitViewModel(scope);

      scope.$watch('person', function fsPersonCardPortraitWatchAction() {
        scope = fsPersonCardPortraitViewModel(scope);
      });
    },
